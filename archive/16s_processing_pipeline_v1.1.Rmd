...
## A Note About Mapping Files Before You Begin

Mapping files need to be in a very specific format for the program to be able to read them in correctly. You can find all of the details [here](qiime.org/documentation/file_formats.html) or [here](https://gls9000.gitbooks.io/qiime-tutorial-for-illumina-paired-end-sequences/content/1_preparing_the_data/14_preparing_a_mapping_file.html). The long-and-short of it is that your mapping file may not pass QC for a number of reasons (one of the main reasons the pipeline will break within the first 10 minutes). The Rules are:

- The file should be named with the same date as the run (so `Nextseq_YYMMDD_mapping.txt` matching `YYMMDD_Nextseq500_...`)
- Should be a tab-delimited file (txt).
- The first variable needs to be "#SampleID" (with the hashtag/pound sign, and all applicable capitalization)
- Second and third columns should be BarcodeSequence and LinkerPrimerSequence, in that order, though LinkerPrimerSequence contents don't matter (I believe). Barcode sequence *definitely* matters.
- No duplicate column names.
- While periods (.) are okay, you can't use underscores (_), dashes(-), or any other special characters in the file.
- Though this won't break the pipeline, it may help to avoid sample names that start with a number due to some R incompatibilities (which should be taken care of in the scripts, but you never know...)
- We have also discussed adding other descriptive columns to the mapping file including final DNA concentration or 16S copy number, just as additional QC checks if we need to make sure a sample's composition is consistent with other measured factors from processing.
- Finally, all negative controls and empty wells should use "NTC" and "EMTPY" respectively in the SampleType variable. This is the variable used in the Negative Control filtering step to identify negative controls in the dataset.
...
An example of an older mapping file.

```{r, echo=FALSE, results='asis', message=FALSE}
library(knitr)
library(kableExtra)
library(dplyr)
map <- read.table("/data/NextSeq_data/NextSeq_Processed/mapping_files/Nextseq_190826_mapping.txt", header=TRUE, check.names=F, sep="\t", comment="")
map %>% 
  kable(format = "html",escape=F) %>% 
  kable_styling("striped", full_width=F) %>%
  add_header_above(c("Table 3"=nrow(.))) %>% 
  scroll_box(width = "100%", height = "3in")
```

## Processing a Raw NextSeq Run into an ASV table

Before you begin, if you have a SampleSheet.csv file in the raw run files, Katie would suggest deleting it. Katie has encountered unexpected bugs in the past due to this file.
